...
public class ResultsGUI extends JFrame {
	...
	private void searchResults() {
		...
		BoxNode sortedSearch= BoxGUI.sortedList.first.next;

		while(sortedSearch != null) {
			
			if((sortedSearch.data).getLength()>=length && (sortedSearch.data).getWidth()>=width && (sortedSearch.data).getHeight()>=height) {
				sorted.append(sortedSearch.data.toString()+"\n");
			}

			
			sortedSearch = sortedSearch.next;
